...
@pytest.mark.asyncio
async def test_redeem(web3, user_account):
    ...
    before_cooldown = await stake_token_contract.get_staker_cooldown(
    ...
    (
        ...
        end_of_cooldown,
        _,
    ) = await stake_token_contract.get_staker_cooldown(
        user_address=user_account.address
    )
    ...
    stata_balance = await stake_stata_contract.balance_of(
